...
if ( ! class_exists( 'STM_Customizer' ) ) {
	class STM_Customizer {
		...
		public static function stm_enqueue_custom_fonts() {
			...
			wp_enqueue_style('stm-custom-fonts', $request, array(), SMARTY_THEME_VERSION );
		}
		...
		public static function customizer_preview_scripts() {
			wp_enqueue_script( 'stm-customizer-preview.js', SMARTY_CUSTOMIZER_URI . '/'.smarty_get_university_mode().'/assets/'.smarty_get_university_mode().'/js/customizer-preview.js', array( 'jquery', 'customize-preview' ), SMARTY_THEME_VERSION, true );
		}
		...
		public static function admin_enqueue_scripts() {
			...
			wp_enqueue_script( SMARTY_THEME_SLUG . '-customizer.js', SMARTY_CUSTOMIZER_URI . '/'.smarty_get_university_mode().'/assets/'.smarty_get_university_mode().'/js/customizer.js', array( 'customize-controls', 'wp-color-picker' ), SMARTY_THEME_VERSION, true );
			...
			if( ! empty( $icons['icons'] ) ){
				...
				wp_localize_script( SMARTY_THEME_SLUG . '-customizer.js', 'stm_icons_array', $fonts );
			}
			...
			switch ( $field_data['type'] ) {
				...
				case 'stm-select-bg':
					self::$customizer->add_control(
						new STM_Customizer_Background_Control (
							self::$customizer,
							$field,
							$field_data
						)
				...
				case 'stm-select-icons':
					self::$customizer->add_control(
						new STM_Customizer_Select_Icons_Control(
							self::$customizer,
							$field,
							$field_data
						)
