...
if( ! function_exists('smarty_event_member_info') ) {
	...
	function smarty_event_member_info($data ){
		...
		$event_member_data['post_title'] = $data['name'] . esc_html__( ' joined to the event: ', 'smarty' ) . '"' . get_the_title( $data['event_id'] ) . '"';
	...
	if( $sidebar != 'hide' ) {
		...
		if( $sidebar == 'left' ) {
			...
			if( !empty( $class_custom['sidebar_before'] ) ) {
				...
				if( is_singular('stm_donation') || is_singular('stm_event') || is_singular('post') || get_post_type() == 'post' ) {
					$layout['sidebar_before'] .= '<div class="col-lg-3 col-md-4 col-lg-pull-3 col-md-pull-8 hidden-sm hidden-xs">';
				} else {
