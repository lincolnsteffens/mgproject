		...
		<div class="entry-header__heading">
			...
			<ul class="entry-meta">
				...
				<?php
					if ( ! is_multi_author() ) {
						printf( '<li><span class="byline"><span class="author vcard"><span class="stm-icon stm-icon-user"></span>%1$s <span class="screen-reader-text">%2$s </span><a class="url fn n" href="%3$s">%4$s</a></span></span></li>',
							_x( 'Posted by:', 'Used before post author name.', 'smarty' ),
                ...
                <?php if( !empty( $event_categories_list ) ) : ?>
                    <li><span class="stm-icon stm-icon-folder"></span><?php echo esc_html__( 'Category:', 'smarty' ); ?> <span class="cat-links"><?php echo wp_kses_post( $event_categories_list ); ?></span></li>
                <?php endif; ?>
			</ul>
		</div>
	</header>
			...
			<li>
				...
				<ul class="event-info__datetime">
					...
					<?php if( !empty( $stm_event_time_text ) ) : ?>
						<li><span class="stm-icon stm-icon-clock"></span><?php echo esc_html( $stm_event_time_text ); ?></li>
					<?php else: ?>
						<?php if( !empty( $stm_event_time_start ) || !empty( $stm_event_time_end ) ) : ?>
							<li><span class="stm-icon stm-icon-clock"></span><?php echo (( $stm_event_time_end ) ? esc_html( $stm_event_time_start . ' ' . esc_html__('to', 'smarty') . ' ' . $stm_event_time_end ) : esc_html( $stm_event_time_start ) ); ?></li>
						<?php endif; ?>
					<?php endif; ?>
				</ul>
			</li>
